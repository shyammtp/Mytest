...
class Blocks_Front_Account_Appointments_Patient extends Blocks_Front_Abstract
{
	...
	public function getPatientAppointments()
	{
		$select = DB::select("*")->from(App::model('core/appointments')->getTableName())
						->where('user_id','=',$this->getUsers()->getId());
		switch($this->getRequest()->query('btype')) {
			case "tomorrow":
			...
			case "yesterday":
				...
				break;
			case "date":
					$date = Arr::get($this->getRequest()->query(),'bdt',date("Y-m-d"));
	...
	{
		return Arr::get($this->getRequest()->query(),'blimit',10);
	}
	
	public function getCurrentPage()
    {
        $currentpage = Arr::get($this->getRequest()->query(),'bpage',1);
