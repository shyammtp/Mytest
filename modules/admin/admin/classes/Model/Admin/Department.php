 ...
 */
class Model_Admin_Department extends Model_Core_Department {

    public function validate($data)
    { 	
        $validate = Validation::factory($data);
        $validate->label('department_name',__('Department'))
                 ->rule('department_name','language_not_empty',array(':value',array(Model_Core_Language::DEFAULT_LANGUAGE))); 
	...
	{ 
		...
		foreach($spreadsheet as $key => $sheet) {
			...
			$data = array();
			try {
				$department = App::model('admin/department',false);
