...
class Controller_Admin_Configuration extends Controller_Core_Admin {
	...
	public function action_save()
	{
		...
		$removeFields = array('return_url','_wysihtml5_mode','config_fields');
		$configfields = array();
				...
				if($validation = $validate->setData($data)->validate()) {
					...
					foreach($data as $name => $value) {
						...
						if(isset($configfields[$name]) && isset($configfields[$name]['source_model'])) {
							$sourcemodel = App::model($configfields[$name]['source_model'])->setConfigModel($model);
							continue;
						}
					...
					App::model('admin_session')->setDatas('errors',$validate->getErrors());
