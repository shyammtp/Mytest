...
class Blocks_Admin_City_Countrycityarea extends Blocks_Admin_Abstract
{
	protected $_area;
    ...
    { 
		if(!$this->_area){
			$this->_area = App::model('admin/area/settings')->load($this->getRequest()->query('id'));
		}
		return $this->_area;
    }
    ...
    { 
		$country = DB::select('main_table.country_id','lng_table.country_name')->from(array(App::model('admin/country/settings')->getTableName(),'main_table'))
    ...
    { 
		$city = DB::select('main_table.city_id','lng_table.city_name')->from(array(App::model('admin/city/settings')
