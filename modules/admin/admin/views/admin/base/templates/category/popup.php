...
<?php 	$this->getRequest()->query('parent');  ?>
<div class="row">
		<div class="col-md-12">
	<!-- Nav tabs -->
	<ul class="nav nav-tabs"></ul>
		...
		<form method="post" class="form-horizontal tab-form"  accept-charset="UTF-8"  id="category_form_val" >
	 
		<div class="tab-content">
		<input type="hidden" name="category_id" value="<?php echo $this->getRequest()->query('id');?>" />
		<input type="hidden" name="parent_id" value="<?php echo $this->getRequest()->query('parent');?>" />
		 <?php echo $this->childView('form');?>
				<div class="panel-footer">
					<button type="button" id="category_form" class="btn btn-primary mr5"><?php echo __('Save');?></button>
				</div>
		 </div>
	...
	$( "#category_form" ).live('click',function( event ) {
		...
		$.ajax({
			...
			if(d.sucess){
				...
				$('#category_form_val').trigger("reset");
			}
			}
		});
...
<script type="text/javascript" src="<?php echo App::getBaseUrl('');?>assets/js/jquery.validate.min.js"></script>
