...
<?php $customer = App::model('admin/session')->getCustomer(); ?>
	...
	<li>
	 <div class="prd_dashbb" <?php if($i==1){ ?> style=" border-top:none;" <?php } ?>>
		<div class="prd_dash_top pad10 show_details" id="message_id-<?php echo $message['primary_id']; ?>">
			<div class="prod_dash_left" >
				<span class="margr5 left_name"><a title="<?php echo $message['from']; ?>"><?php echo $message['from']; ?></a><br><span class="clr4 fnt10 replies"><?php echo date("M-d-Y h:i A", strtotime($message['post_date'])); ?></span></span>
				<p class="clr5 widtaut margl5 right_name"><i class="backr1 margr5 margl5" style="background: none repeat scroll 0 0 <?php echo $message['color_code']; ?>;"><?php echo $message['subject_index']; ?></i><?php  if($message['reply_id']!='0'){ echo '<b>Re</b>: '.$message['replymessage'].' - '.$message['subject']; } else { echo $message['subject']; } ?></p>
			</div>
