...
class Blocks_Store_Area_Edit extends Blocks_Store_Abstract
{
	protected $_area;
    ...
    { 
		if(!$this->_area){
			$this->_area = App::model('store/area/settings',false)->loadAllLanguages($this->getRequest()->query('id'));
		}
		return $this->_area;
    }
    ...
    { 
		$options=array();
		$area = DB::select('main_table.area_id','lng_table.area_name')->from(array(App::model('store/area/settings')->getTableName(),'main_table'))
