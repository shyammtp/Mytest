...
?>
<?php $reply = App::model('store/notification/reply')->getReplies($notification['msg_id']);?>
<div id="container">
<div class="loader">
	<center>
	...
	<div class="search_cont">
		<div class="serch_cont_lft pull-left">
			<p class="clr6 widtaut"><span class="fnt15"><?php echo $notification['subject']; ?></span><span class="backr1 margr5 margl5" style="background: none repeat scroll 0 0 <?php echo $subject_type['color_code']; ?>;"><?php echo $subject_type['subject_index']; ?></span></p>
		 </div>
		 <div class="pull-right serch_cont_rgt">
			  <?php /*
			 <a class="widtaut margr5 margb5 reply-position"><i class="margr5 fa fa-reply-all"></i><?php echo __('Reply');?></a>
	 ...
	 <div class="prd_dashbb notificat replyresponse-<?php echo $replymessage['reply_id']; ?>">
		...
		<?php $sessionusermodel = App::model('user',false)->selectAttributes('*')->setLanguage(App::getCurrentLanguageId())->setConditionalLanguage(true)->load($sessionid);?>
         ...
         <div class=" panel-footer paddin">
			...
			<button class="btn btn-primary mr5 reply pull-left" id="reply-position"><?php echo __('Reply');?></button>
			<button class="btn btn-default  mr5 pull-left" type="button" onclick="setLocation('<?php echo $this->getUrl("admin/dashboard/index");?>')"><?php echo __('Cancel');?></button>
         </div>		
	 </div>	 
	 </div>	
<script>
