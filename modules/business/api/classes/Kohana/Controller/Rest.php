...
abstract class Kohana_Controller_Rest extends Controller {
	...
	public function before()
	{
		...
		if($this->request->query('format'))  {
			$this->output_format = $this->request->query('format');
		} else {
	...
	{
		$this->_suppress_response_codes = true;
	...
	private function _format_html($data = array())
	{
		...
		try
		{
			Block::init('store','base');
			$block = Block::factory('customer/managecustomer');
			$content = $block->getBlock('listing');
